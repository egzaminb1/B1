import trackA1_1 from '../../assets/audio/catA1_1.mp3';

export const QuizA1_1 = {
    tracks:[trackA1_1],
    header:['Proszę uważnie słuchać tego nagrania i zaznaczyć właściwe odpowiedzi.',
            'Uwaga! Nagranie zostanie odtworzone tylko jeden raz.'],
    template:{  point:0.5,
                iterate:1,
                pH:'1_1 Ta wypowiedź jest typowa:',
                p:[
                    {word:'w tramwaju',cls:''},
                    {word:'w szkole',cls:'bold'},
                    {word:'na uniwersytecie',cls:''}
                ]    
            },
        card:[
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w samolocie' ,cls:'' ,answer:false},
                    {word:'w autobusie' ,cls:'' ,answer:true},
                    {word:'w kinie' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w banku' ,cls:'' ,answer:false},
                    {word:'w sklepie' ,cls:'' ,answer:false},
                    {word:'na poczcie' ,cls:'' ,answer:true}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'na poczcie' ,cls:'' ,answer:false},
                    {word:'na stacji benzynowej' ,cls:'' ,answer:true},
                    {word:'w biurze' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w szkole' ,cls:'' ,answer:false},
                    {word:'na poczcie' ,cls:'' ,answer:true},
                    {word:'w biurze' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'u lekarza' ,cls:'' ,answer:false},
                    {word:'w restauracji' ,cls:'' ,answer:true},
                    {word:'w dyskotece' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'u kosmetyczki' ,cls:'' ,answer:false},
                    {word:'u fryzjera' ,cls:'' ,answer:true},
                    {word:'w solarium' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'na dworcu kolejowym' ,cls:'' ,answer:false},
                    {word:'na dworcu autobusowym' ,cls:'' ,answer:false},
                    {word:'na lotnisku' ,cls:'' ,answer:true}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w bibliotece' ,cls:'' ,answer:true},
                    {word:'w księgarni' ,cls:'' ,answer:false},
                    {word:'w sklepie' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'u lekarza' ,cls:'' ,answer:true},
                    {word:'u ortopedy' ,cls:'' ,answer:false},
                    {word:'u dentysty' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w tramwaju' ,cls:'' ,answer:false},
                    {word:'w pociągu' ,cls:'' ,answer:true},
                    {word:'w samolocie' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w aptece' ,cls:'' ,answer:true},
                    {word:'u terapeuty' ,cls:'' ,answer:false},
                    {word:'w gabinecie lekarskim' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'u fotografa' ,cls:'' ,answer:true},
                    {word:'w sklepie' ,cls:'' ,answer:false},
                    {word:'u fryzjera' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w przedszkolu' ,cls:'' ,answer:false},
                    {word:'w domu' ,cls:'' ,answer:false},
                    {word:'w szkole' ,cls:'' ,answer:true}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'na ulicy' ,cls:'' ,answer:true},
                    {word:'w samolocie' ,cls:'' ,answer:false},
                    {word:'na parkingu' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w hotelu' ,cls:'' ,answer:false},
                    {word:'w sklepie' ,cls:'' ,answer:false},
                    {word:'w banku' ,cls:'' ,answer:true}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w operze' ,cls:'' ,answer:false},
                    {word:'w muzeum' ,cls:'' ,answer:true},
                    {word:'w filharmonii' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'w sklepie obuwniczym' ,cls:'' ,answer:false},
                    {word:'w sklepie odzieżowym' ,cls:'' ,answer:true},
                    {word:'w supermarkecie' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź to:',
                p:[
                    {word:'skarga' ,cls:'' ,answer:false},
                    {word:'podziękowanie' ,cls:'' ,answer:false},
                    {word:'oburzenie' ,cls:'' ,answer:true}
                ]
            },
            {
                pH:'Ta wypowiedź to:',
                p:[
                    {word:'komplement' ,cls:'' ,answer:false},
                    {word:'skarga' ,cls:'' ,answer:true},
                    {word:'życzenia' ,cls:'' ,answer:false}
                ]
            },
            {
                pH:'Ta wypowiedź jest typowa:',
                p:[
                    {word:'na dworcu autobusowym' ,cls:'' ,answer:true},
                    {word:'na dworcu kolejowym' ,cls:'' ,answer:false},
                    {word:'na stacji metra' ,cls:'' ,answer:false}
                ]
            }

        ]
}